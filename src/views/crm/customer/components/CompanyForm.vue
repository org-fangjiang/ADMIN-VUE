...
<script lang="ts">
  ...
  import { addCompany, getCompanyById } from '/@/api/customer/crmCompany/company';
  ...
  import { CompanyConst, CompanyModel } from '/@/api/customer/crmCompany/model/companyModel';
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      //验证规则
      const rules = reactive(CompanyConst._RULES);
      ...
      onMounted(async () => {
        loading.value = true;
        if (props.id) {
          const { content } = await getCompanyById(props.id);
      ...
      return {
        ...
        labelCol: { span: 6 },
        wrapperCol: { span: 12 },
        updateFields: CompanyConst._UPDATE_FIELDS,
        liveInChange,
        workInChange,
        changeProvince,
        changeCity,
