    ...
    <Table :columns="brandColumns" :data-source="list" rowKey="id" :pagination="false">
      ...
      <template #operation="{ text: line }">
        ...
        <Dropdown placement="bottomCenter" trigger="click">
          <Button
            v-if="hasPermission([brandConst._PERMS.DELETE, brandConst._PERMS.UPDATE])"
            type="link"
          ...
          <template #overlay>
            <Menu mode="horizontal" @click="action">
              <MenuItem
                v-if="hasPermission(brandConst._PERMS.DELETE) && deleteOrEnable(line.state)"
                :key="0"
                :data-id="line.id"
              ...
              <MenuItem
                v-if="hasPermission(brandConst._PERMS.UPDATE) && !deleteOrEnable(line.state)"
                :key="1"
                :data-id="line.id"
                :class="`${prefixCls}-action-menu-item`"
              >
              ...
              <MenuItem
                v-if="hasPermission(brandConst._PERMS.UPDATE)"
                :key="2"
                :data-id="line.id"
                :class="`${prefixCls}-action-menu-item`"
              >
