  ...
  <div :class="prefixCls" class="relative w-full h-full px-4">
    ...
    <Table :columns="estateCompanyColumns" :data-source="list" rowKey="id" :pagination="false">
      <template #state="{ text: state }">
        <span>
          <Tag :color="estateCompanyConst.STATES[state].color">
            {{ estateCompanyConst.STATES[state].label }}
          </Tag>
        </span>
      </template>
          ...
          <template #overlay>
            <Menu mode="horizontal" @click="action">
              <MenuItem :key="0" :data-id="line.id" :class="`${prefixCls}-action-menu-item`">
                ...
                <Button
                  :class="prefixCls"
                  v-auth="estateCompanyConst._PERMS.DELETE"
                  type="link"
                  size="small"
    ...
    <Modal
      ...
      :destroyOnClose="true"
    >
      <EstateCompanyForm v-if="drawerParam.state === '0'" :id="drawerParam.id" />
    </Modal>
...
<script lang="ts">
  ...
  import EstateCompanyForm from './components/estateCompanyForm.vue';
  import {
    deleteEstateCompany,
    getEstateCompanies,
    reEnableEstateCompany,
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const getList = async () => {
        loading.value = true;
        let result: BasePageResult<EstateCompanyModel> | undefined;
        try {
          result = await getEstateCompanies(condition, {
            pageSize: pageParam.size,
...
<style lang="less">
  @prefix-cls: ~'@{namespace}-estateCompany';
  @dark-bg: #293146;
