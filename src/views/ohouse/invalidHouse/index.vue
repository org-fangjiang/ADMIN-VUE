  ...
  <div>
    ...
    >
      ...
      <template #operation="{ text: line }">
        <Button :class="prefixCls" type="link" size="small" @click="updateHouse(line)">
          {{ t('host.action.see') }}
        </Button>
        <Button :class="prefixCls" type="link" size="small" @click="maintainHouse(line)">
          维护
        </Button>
      </template>
    </Table>
...
<script lang="ts">
  ...
  import { getOHouseList, maintain } from '/@/api/ohouse/house/house';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      // 维护
      const maintainHouse = async (line) => {
        try {
          await maintain(line.id);
          success('成功', '申请维护成功');
          const result = await getList();
          processListByLine(result, list, total);
        } catch (error) {
